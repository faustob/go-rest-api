...
func main() {
	// Register the OpenTelemetry SDK as the global provider before anything is instrumented.
	...
	if err != nil {
		log.Printf("### 📡 OTel: failed to initialise telemetry, continuing without it: %s", err)
	}
	...
	defer func() {
		...
		if err := otelShutdown(shutdownCtx); err != nil {
			log.Printf("### 📡 OTel: shutdown error: %s", err)
		}
	}()
	...
	// OpenTelemetry: server spans + semconv HTTP server metrics.
	// Registered after Recoverer (panics still recovered) and BEFORE the JWT
	// validator below, so denied/short-circuited requests are still counted.
