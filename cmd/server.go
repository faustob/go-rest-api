...
func main() {
	...
	// Register the OpenTelemetry SDK globally, endpoint is driven by OTEL_EXPORTER_OTLP_ENDPOINT
	otelShutdown, otelErr := telemetry.InitOTel(context.Background(), serviceName, version)
	if otelErr != nil {
		log.Printf("### 📡 OTel: failed to initialise, continuing without it: %s", otelErr)
	}
	...
	// OpenTelemetry: server spans + http.server.request.duration, then route/outcome metrics.
