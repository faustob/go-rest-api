...
func main() {
	...
	// OpenTelemetry HTTP server instrumentation (spans + http.server.request.duration)
	router.Use(otelchi.Middleware(serviceName, otelchi.WithChiRoutes(router)))
	// Custom telemetry: request outcome counter, saturation gauges, SLO span events
